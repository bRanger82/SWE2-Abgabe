...
/* Suchen im binaeren Baum */

KNOTEN *suchen(KNOTEN *zeiger, int key)  {
   if(zeiger == NULL) {
      return NULL;              // nicht gefunden
   }
   else if(zeiger->wert == key) 
      return zeiger;            // gefunden
   else if(zeiger->wert > key)  // weitersuchen
      return suchen(zeiger->links,  key);
   else 
      return suchen(zeiger->rechts, key);
   return (zeiger);
}
